    ...
    def convert(self, value):
        ...
        if isinstance(value, six.string_types):
            ...
            if match:
                ...
                raise SearchFieldError("Date provided to '%s' field doesn't appear to be a valid date string: '%s'" % (
...
class FacetField(SearchField):
    ...
    def handle_facet_parameters(self, kwargs):
        ...
        if not kwargs.get('null', True):
            raise SearchFieldError(
                "FacetField (%s) does not accept False for the 'null' argument." % self.instance_name)

        if not kwargs.get('indexed', True):
            raise SearchFieldError(
                "FacetField (%s) does not accept False for the 'indexed' argument." % self.instance_name)
